...
from config import API_HOST, API_PORT, UPLOAD_DIR
...
class AnswerBody(BaseModel):
    session_id: str
    user_id: str = "default_user"
    transcript: str | None = None  # if no audio, use text directly
...
async def analyze(
    ...
    try:
        ...
        raise HTTPException(500, "Analysis failed. Please try again.") from e
    finally:
...
async def analyze_stream(
    ...
    async def _generate():
        try:
            ...
            yield (
                json.dumps({"status": "error", "detail": "Analysis failed. Please try again."})
                + "\n"
            )
        finally:
            file_path.unlink(missing_ok=True)
...
async def interview_answer(
    ...
    db_session_id: str | None = Form(None),
):
    """
    ...
    try:
        ...
        raise HTTPException(500, "Interview processing failed. Please try again.") from e
    finally:
...
async def reset_sessions(user_id: str):
    ...
    try:
        ...
        raise HTTPException(500, "Could not reset session history") from e
...
async def parse_resume(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(500, "Could not parse resume") from e
...
def _build_analyze_response(session_id: str, result: dict, report: dict | None) -> dict:
    """
    ...
    audience_engagement = max(
        0, min(100, round(confidence_score * 0.4 + pace_sc * 0.3 + filler_sc * 0.3))
    )
    ...
    detected_lang_label = (
        _code_to_label.get(raw_lang_code, raw_lang_code) if raw_lang_code else None
    )
    ...
    filler_set = (
        {fw["word"].lower() for fw in (report.get("filler_words") or [])} if report else set()
    )
